...
/* CRC24 as the game computes it: the OpenPGP polynomial and starting value.
 * Bit at a time rather than table driven — a 256 entry table is a kilobyte of
 * constant data to save a few microseconds on a lookup that happens once per
 * resource name, and this engine counts kilobytes. */
#define CRC24_POLYNOMIAL 0x864CFBUL
#define CRC24_INITIAL 0xB704CEUL

/* CRC32 in its MPEG-2 form. Same polynomial as the familiar one, but the input
 * and output are not reflected and the result is not inverted, which makes it a
 * different function producing entirely different numbers. */
