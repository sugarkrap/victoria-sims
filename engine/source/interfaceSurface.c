 ...
 *
 * Adding the top byte back before shifting is the standard way round it and is
 * exact for every one of the 65536 pairs. Dividing by 255 costs more than
 * everything else in the inner loop put together on hardware with no divider,
 * which is most of the device ladder below the reference handheld. */
...
/* One premultiplied source over one destination pixel. */
...
/* Clips a span to the surface. Answers false when nothing of it is left, which
 * is a perfectly ordinary outcome for a menu laid out wider than the window and
 * is counted rather than treated as an error. */
                            ...
                            InterfaceColor color)
{
    ...
    {
        /* Thicker than the thing it surrounds, so it is the thing it
           surrounds. Better than drawing four overlapping bars that add their
           alpha together at the corners and come out darker there. */
    ...
    interfaceSurfaceFill(surface, left, top + (Integer32)(height - thickness), width, thickness,
                         color);
    /* The sides stop short of the bars above and below, so no pixel is covered
       twice — which with a translucent border shows as four darker corners. */
        ...
        if (glyph->widthInPixels > 0U && glyph->heightInPixels > 0U)
        {
            ...
            for (row = 0U; row < glyph->heightInPixels; row++)
            {
                ...
                for (column = 0U; column < glyph->widthInPixels; column++)
                {
                    ...
                    /* The glyph's coverage and the caller's opacity multiply:
                       a half-covered pixel of a half-transparent label is a
                       quarter, which is what both of them mean together. */
                           ...
                           Unsigned32 sourceWidth, Unsigned32 sourceHeight)
{
    ...
    for (row = fromY; row < toY; row++)
    {
        /* Which source row this destination row samples. Computed from the
           destination position rather than stepped, so a rectangle clipped at
           the top still samples the part of the image that belongs there. */
