...
/* Every block type that can turn up inside a CRES. Most are here only so the
 * walk can measure them and move on. */
...
/* A data list extension holds a value that may be a list of values. Nesting is
 * bounded here rather than trusted: a file claiming a thousand levels would
 * otherwise recurse a thousand frames deep, and this engine has one stack and
...
/* A one byte length and that many characters. Not the seven bits at a time
 * form used everywhere else — the tag extension writes this one, and reading
 * the wrong shape moves everything after it. */
...
/* cObjectGraphNode: the dependencies of a block, and from version 4 its tag. */
...
/* cCompositionTreeNode: an object graph node, then the blocks hanging below
 * this one. Those references are the tree. */
                                ...
                                Unsigned32 ownBlockIndex, NodeEdge *edges, Unsigned32 *edgeCount)
{
    ...
    for (index = 0U; index < count; index++)
    {
        ...
        /* Only references to blocks in this same collection are part of the
         * tree. An external one points at another file, which is somebody
         * else's problem. */
...
/* The body of a cTransformNode, with its type prefix already read or not. */
...
/* cBoundedNode wraps a cTransformNode; cRenderableNode wraps that. Both add a
 * little of their own, and both must be read in full to find what follows. */
static void readBoundedNode(ResourceCursor *cursor, TransformNode *node, Unsigned32 ownBlockIndex,
                            NodeEdge *edges, Unsigned32 *edgeCount)
{
    resourceCursorReadTypeInformation(cursor, NULL_POINTER); /* cBoundedNode */
    resourceCursorReadTypeInformation(cursor, NULL_POINTER); /* cTransformNode */
    readTransformBody(cursor, node, ownBlockIndex, edges, edgeCount);
    (void)resourceCursorReadUnsigned8(cursor); /* always written as zero */
}
                               ...
                               NodeEdge *edges, Unsigned32 *edgeCount)
{
    ...
    resourceCursorReadTypeInformation(cursor, NULL_POINTER); /* cRenderableNode */
    readBoundedNode(cursor, node, ownBlockIndex, edges, edgeCount);

    (void)resourceCursorReadUnsigned8(cursor); /* part of all render groups */
    count = resourceCursorReadUnsigned32(cursor);
    if (!countFits(cursor, count))
    {
        return;
    ...
    (void)resourceCursorReadUnsigned32(cursor); /* render group */
    (void)resourceCursorReadUnsigned8(cursor);  /* add to display list */
}

/* One value in a data list extension, which may be a list of values. Read only
 * to be got past. */
static void readExtensionValue(ResourceCursor *cursor, Unsigned32 depth)
{
    ...
    default:
        /* A type code with no known width. Stopping is the only safe answer:
         * skipping an unknown number of bytes is how a reader ends up parsing
         * the middle of a structure and believing it. */
        cursor->overran = BOOLEAN_TRUE;
        break;
    }
}

/* Consumes one block. Fills in the node when the block carries a transform.
 * Returns false for a block type whose length cannot be known. */
                         ...
                         Unsigned32 *edgeCount, char *resourceName, MemorySize resourceNameCapacity)
{
    *carriesTransform = BOOLEAN_FALSE;
    ...
    case BLOCK_RESOURCE_NODE:
    {
        ...
        if (hasTree != 0U)
        {
            /* Named through a cSGResource, then the tree of its children. */
            resourceCursorReadTypeInformation(cursor, NULL_POINTER);
        ...
        {
            /* No tree, so the graph node's tag is the only name it has. */
            readObjectGraphNode(cursor, resourceName, resourceNameCapacity);
        }
        (void)resourceCursorReadObjectReference(cursor);
        (void)resourceCursorReadUnsigned32(cursor); /* skin type */
        return BOOLEAN_TRUE;
    }
    ...
    case BLOCK_SHAPE_REFERENCE_NODE:
    {
        ...
        *carriesTransform = BOOLEAN_TRUE;
        ...
        for (index = 0U; index < count; index++)
        {
            ...
            /* The first shape this node names. A node referencing several is
             * rare and the extras are not lost so much as not yet used. */
        ...
        (void)resourceCursorReadUnsigned32(cursor); /* display list flags */
        ...
        if (blockVersion > 19U)
        {
            (void)resourceCursorReadUnsigned32(cursor); /* shape colour */
        }
        return BOOLEAN_TRUE;
    }
    ...
    case BLOCK_DATA_LIST_EXTENSION:
        resourceCursorReadTypeInformation(cursor, NULL_POINTER); /* cExtension */
    ...
    case BLOCK_BONE_DATA_EXTENSION:
        resourceCursorReadTypeInformation(cursor, NULL_POINTER); /* cExtension */
        resourceCursorSkip(cursor, 16UL);                        /* three axes and a factor */
        if (blockVersion > 3U)
        {
            resourceCursorSkip(cursor, 16UL); /* joint orientation */
        }
        return BOOLEAN_TRUE;
                                    ...
                                    MemorySize sizeInBytes)
{
    ...
    for (blockIndex = 0U; blockIndex < collection.blockCount; blockIndex++)
    {
        ...
        /* Written into a scratch node when the store is full, so a model with
         * more parts than fit is still walked to the end — the blocks after it
    ...
    /* The tree, once every block has a number. An edge names blocks; the nodes
     * are a subset of those, so both ends have to be looked up. */
                                   ...
                                   Real32 *matrix)
{
    ...
    /* Bounded by the number of nodes: a file whose parents form a cycle would
     * otherwise walk up forever, and nothing here has verified it does not. */
