...
/* The one property worth keeping out of the forty a retail material carries. */
#define BASE_TEXTURE_PROPERTY "stdMatBaseTextureName"

/* Below this the layout differs, and there is nothing on hand to check a guess
 * against. Refused by version rather than misread. */
                                ...
                                MemorySize sizeInBytes)
{
    ...
    /* A property is two strings, so at least two bytes. A count larger than what
     * is left is not a count. */
    ...
    for (index = 0U; index < propertyCount; index++)
    {
        ...
        /* Every value is read whether or not it is wanted: the list has to be
         * walked to reach the textures after it, and skipping a string means
         * reading its length anyway. */
    ...
    /* A material with no base texture property but a texture list still has a
     * texture. Falling back rather than reporting none keeps a part painted. */
