 ...
 *
 * Named for what they are for rather than for what colour they are, so a change
 * of mind about the look is a change here and nowhere else. Everything is
 * translucent over the scene except the text, because a debug menu that hides
 * the model it is describing has defeated itself. */
                         ...
                         Unsigned32 availableHeight)
{
    ...
    if (layout->width < layout->sidebarWidth + layout->tileWidth + (2U * MENU_MARGIN) ||
        layout->height < layout->headerHeight + layout->tileHeight + layout->pagerHeight +
                             (2U * MENU_MARGIN))
    {
        /* Not enough room for even one tile. The chrome is still drawn and says
           so — a menu that vanished on a small window would look like a menu
           that had broken. */
...
/* Where one page button sits. Shared by the drawing and the hit test, which is
   the whole reason it is a function — a button drawn in one place and tested in
   another is a button that does not work, and the bug is invisible in code. */
                                      ...
                                      Integer32 x, Integer32 y)
{
    ...
    for (position = 0U; position < perPage; position++)
    {
        ...
        /* The tile is tested at the size it is DRAWN, not at the size of the
           cell it lives in: the gap between two tiles belongs to neither, and a
           click there should choose nothing rather than whichever neighbour the
           arithmetic rounded towards. */
...
/* The stand-in for a thumbnail nobody has yet.
 *
 * A checkerboard, which is the universal way of saying "there is no picture
 * here" and cannot be mistaken for one — and in two shades derived from the
 * row's name, so twenty tiles of stand-in are still told apart at a glance.
 * When a real picture turns up for a row it simply replaces this. */
                       ...
                       InterfaceMenuThumbnail thumbnail, void *thumbnailContext)
{
    ...
    for (index = 0U; index < perPage; index++)
    {
        ...
        /* The cursor is a ring round the whole tile and what is in effect is a
           ring round the picture, so a tile that is both shows both. Two things
           drawn the same way would make "the one I am looking at" and "the one
           I am wearing" indistinguishable, which is the confusion the menu
           exists to prevent. */
    ...
    /* The pager. Both buttons are always drawn, and both are drawn dim when
       there is nowhere to go — a button that disappears at the end of a list
       moves the other one, and the pointer ends up over neither. */
