...
/* How far past the first descriptor to look before giving up on finding a
 * terminator. The set ends within a handful of sectors on any real disc. */
...
/* The three escape sequences that mark a supplementary descriptor as Joliet,
 * differing only in the UCS-2 level they claim. */
...
/* Writes one code point as UTF-8. Returns how many bytes it took, or zero if it
 * would not fit. Joliet is UCS-2, so nothing above the basic multilingual plane
 * can arrive here. */
...
/* Decodes a directory identifier into destination, dropping the ";1" version
 * suffix ISO 9660 appends to file names. Returns the length written, or zero if
 * it would not fit. Directories never carry a version suffix. */
...
/* Joins a directory's path and a child's name into the pool, and returns where
 * it landed. Negative when the pool is full. */
                               ...
                               Unsigned32 fileLimit)
{
    ...
    /* The root's path is the empty string, and it lives at offset zero so every
     * other path can be built by joining onto something. */
    reader->pathPool[0] = '\0';
...
static DiscReadStatus stepDescriptors(DiscReader *reader)
{
    ...
    {
        /* Preferred over the primary tree wherever it exists: it is where the
         * real file names live. */
...
static DiscReadStatus stepDirectories(DiscReader *reader)
{
    ...
    if (read == VIRTUAL_READ_OUT_OF_RANGE)
    {
        /* A directory claiming to live past the end of the image. Skipping it
         * beats abandoning the disc, and beats looping forever asking the host
         * for bytes that cannot exist. */
        reader->pendingCount--;
        return DISC_READ_PENDING;
    }
    if (read != VIRTUAL_READ_OK)
    ...
    /* Taken off the stack only once its bytes are in hand, so a PENDING read
     * resumes on the same directory. */
    reader->pendingCount--;
    ...
    while (cursor < length)
    {
        ...
        if (recordLength == 0UL)
        {
            /* Records never straddle a sector boundary; a zero length means the
        ...
        /* Identifiers of a single NUL or 0x01 are "." and "..". */
...
DiscReadStatus discReaderRunToCompletion(DiscReader *reader)
{
     ...
     * should not have used this. One step per sector of descriptors plus one per
     * directory leaves generous room. */
