...
/* A DBPF header is ninety six bytes, and the three numbers that matter sit at
 * fixed places in it. Read on its own so a package's contents are never
 * touched: the whole point of this is to learn what is in six hundred files
 * without reading six hundred files. */
...
/* An index larger than this is not indexed. A retail objects.package has a big
 * one, but not this big, and a number this size means the header was misread
 * rather than that the file is unusual. */
...
ResourceIndexStatus resourceIndexStep(ResourceIndex *index)
{
    ...
    if (index->readingHeader)
    {
        ...
        {
            /* Not here yet. The file index is untouched, so the next step asks
...

    {
        ...
        /* Entry width from the arithmetic rather than the version field, the
         * same way the package reader establishes it — both layouts occur, and
         * the division has proved the more reliable of the two. */
        ...
        index->filesIndexed++;
        /* The index bytes were only ever needed to be read. */
        memoryArenaRewindToMarker(index->arena, marker);
        return RESOURCE_INDEX_WORKING;
    }
}
                                            ...
                                            Unsigned32 instanceIdentifierHigh)
{
    ...
    for (which = 0U; which < index->count; which++)
    {
        ...
        /* A package whose entries carry no high word stores zero for it, and a
         * name always hashes to something. Matching those would be matching on
         * half a key, so an entry with no high word is only accepted when the
