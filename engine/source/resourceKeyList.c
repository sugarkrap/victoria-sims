...
/* Words, little endian, read a byte at a time because the resource sits at
   whatever offset the package put it at. */
                                          ...
                                          MemorySize sizeInBytes)
{
    ...
    {
        /* No sentinel, so the word already read is the count and the list is the
           older layout. Not a fallback for something unrecognised — it is how
           version 1 is spelled. */
        list->version = 1U;
        list->keyCount = first;
    }
    if (overran)
    ...
    /* Refused before anything is written on the strength of it, as every count
       in this engine is. */
    ...
    for (index = 0U; index < list->keyCount; index++)
    {
        ...
        /* The word that only version 2 carries. Reading it on a version 1 list
           would take the next key's type, and every key after the first would
           be wrong while still looking like a key. */
        if (list->version > 1U)
        {
