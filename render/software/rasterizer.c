...
/* Colour is carried through the span as 16.16 fixed point over 0..255. Fixed
   point rather than float because the floor of this project's device ladder
   has no floating point unit at all, and a per-pixel float multiply there is a
   library call. */
                            ...
                            Real32 colorScale)
{
    Real32 screenX[3];
    Real32 screenY[3];
    /* Which input vertex each screen-space slot came from, so colours still
       follow their positions after a winding fix. */
    ...
    {
        /* Y is flipped: normalised device coordinates put positive Y upwards,
           a framebuffer puts row zero at the top. */
    ...
    /* A degenerate triangle covers nothing. A negative area only means the
       other winding, which still has to be drawn: OpenGL ES leaves face
       culling disabled unless asked, and the shader backends therefore draw
       both. Swapping two vertices makes the area positive so the single
       "all weights non-negative" inside test works for either winding. */
    if (area == 0.0f)
    {
        return;
    }
    ...
    for (pixelY = minimumY; pixelY < maximumY; pixelY += 1)
    {
        ...
        /* Find where this scanline enters and leaves the triangle, then hand
           the run to the span filler. Testing edges per pixel here and doing
           the colour work in one tight pass is what makes the inner loop
           vectorisable at all. */
        ...
        {
            /* Colour is linear across the triangle, so it only has to be
               evaluated at the two ends of the run and stepped between them. */
