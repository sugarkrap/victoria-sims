/* NEON span filler. Compiled only when the target actually has NEON; the
   portable implementation in rasterizer.c compiles itself out in that case, so
   exactly one of the two is ever linked.

   The contract is byte-identical output, not merely similar output. Everything
   here is integer arithmetic mirroring the scalar path exactly: the same 16.16
   fixed point, the same arithmetic shift, the same clamp to 0..255. */
                        ...
                        Integer32 redStep, Integer32 greenStep, Integer32 blueStep)
{
    ...
    /* Lane n starts n steps along, and each iteration advances all four lanes
       by four steps. Multiplying rather than accumulating keeps the lanes
       exactly on the values the scalar loop would have produced. */
    ...
    {
        /* Arithmetic shift, matching the scalar >> on a signed value: a
           negative colour must stay negative so the clamp below sees it. */
    ...
    /* Tail. Recomputed from the span start rather than read out of the vector
       registers, so the remainder cannot drift from the scalar result. */
    {
