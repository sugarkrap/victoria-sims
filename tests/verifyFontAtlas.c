/* Checks the glyph atlas, and mostly checks the part of it that goes to disk.
 *
 * Building the sheet is the easy half: if it were wrong the text would look
 * wrong and somebody would say so. Storing and restoring it is the half that
 * fails silently — a block that restores with the wrong metrics draws letters
 * from the right sheet at the wrong coordinates, which is a font that has gone
 * subtly strange rather than a cache that has gone wrong, and the two are not
 * distinguishable by looking.
 *
 * So the cases here are the ones a cache meets in the field: a block from a
 * different font, from a different size, from a build with a different sheet,
 * and a block that was half written because the run that wrote it was killed. */
...
int main(void)
{
    ...
    /* The font the engine carries with it. It has to work with no disc at all,
       because saying "there is no disc" is one of the things it exists for. */
    ...
    /* An atlas with nowhere to put the sheet. Every call still answers, so a
       caller does not have to check whether the atlas exists before using it. */
    {
        FontAtlas unbound;
    ...
    checkThat(&failureCount, "and remembers what it was made from", atlas.sourceMark == 0x1234U);
    {
        ...
        /* A character the font does not cover is drawn as glyph nought, which
           is what the format keeps glyph nought for: the missing-glyph box.
           Drawing nothing there would be worse — a line of text with a silent
           gap in it looks like text that was never written. */
    ...
    /* Built twice over, which is what actually happens: the engine builds from
     * the font it carries so it can say something at once, and builds again
     * from the disc's font when one turns up.
     *
     * A glyph the second build has no pixels for — a space, and every character
     * the disc's font does not cover — kept the FIRST build's coordinates while
     * the sheet under them had been redrawn, and drew a rectangle of whatever
     * letter now lived there. On screen that was a vertical bar between every
     * two words, which reads as a font with a broken space and is nothing of
     * the kind. */
    {
        FontAtlas rebuilt;
    ...
    /* The round trip. This is the whole point of the file: a slow machine
       rasterizes once and reads pixels ever after. */
    ...
    /* Everything a stale or damaged block can be. Each of these draws
       plausible-looking text if it is not caught, which is why they are caught
       here rather than left to be noticed. */
    {
        ...
        /* One pixel changed somewhere in the middle. A cache file left half
           written by a run that was killed is the ordinary case, not the
           exotic one, and half a sheet still looks like a sheet. */
    ...
    /* A sheet with no room. Counted rather than silently short, because an
       atlas quietly missing its widest letters draws text with holes in it. */
    {
        FontAtlas cramped;
